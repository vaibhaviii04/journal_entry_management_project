...
def profile(username):
    if session['username']!=username: flash('Access denied','error'); return redirect(url_for('login'))
    user = get_user(username)
    return render_template('profile.html', username=user['username'], email=user['email'], profile_pic_url=url_for('static',filename='default_profile.png'), account_creation_date="N/A")
